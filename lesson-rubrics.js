  ...
  const RUBRICS = {
    ...
    '1-5': {
      ...
      requiredMeanings: [
        ...
        { id: 'data-minimisation', label: 'remove unnecessary personal or sensitive information', threshold: 0.50, explicitRequired: true, explicitPatterns: [/\b(?:de[ -]?identif\w*|remov\w*|strip\w*|take out)\b.{0,100}\b(?:customer|custmer|name|account|acount|contact|pii)\w*/i,/\bde[ -]?identif\w*\b.{0,100}\b(?:minimum|only|needed)\b/i], examples: ['Remove customer names, account numbers, contact details and other identifying information.', 'Use de-identified examples and include only the minimum information necessary.', 'Share only non-identifying details needed for the task.', 'Strip out names, account numbers and contact details first.', 'Strip out names, account numbers and contact stuff first.', 'Strip out names, account numbers and contact details first, then use only the information needed.', 'De-identify PII and account identifiers and use the minimum data needed.', 'De-ID PII first; minimum coaching data only.'] }
      ],
      supportingMeanings: [
      ...
      blockConditions: [
        ...
        { id: 'sensitive-upload', patterns: ['keep the customer names', 'keep customer names', 'keep the names', 'ai has the full context', 'include customer names', 'include account numbers', 'paste customer details', 'upload customer details', 'copy the raw customer details into the prompt', 'copy customer details into the prompt', 'put customer details into the prompt', 'send customer details to the ai', 'feed customer details into the ai', 'full customer file'] }
      ]
    },
    '4-2': {
      ...common,
      ...
      requiredMeanings: [
        { id: 'stop', label: 'do not use the tool while approval is unclear', explicitRequired: true, explicitPatterns: [/\b(?:do not|don't|dont|will not|won't|wont|would not|can't|cant|cannot|no)\b.{0,45}\b(?:upload|uplod|use|put)\b/i,/\b(?:hold|pause|stop)\b/i], examples: ['I would not upload the lending material yet.', 'Pause the request and do not test the unknown tool.', 'I would not use the unapproved tool.', 'Hold the material until approval is confirmed.', 'Hold the payload until approval is confirmed.', 'Pause before using the tool while approval is unclear.', "Don't upload yet; confirm approval before use."] },
        { id: 'confirm-approval', label: 'confirm whether the tool and use are approved', explicitRequired: true, explicitPatterns: [/\b(?:check|chek|confirm|verify|validate)\b.{0,90}\b(?:approv|aprov|authoris|policy|register|listed)\w*/i], examples: ['Check the approved-tool register or ask the responsible policy owner.', 'Confirm the tool is authorised for this type of information.', 'Validate the AI service against the approved-tool register.'] },
        { id: 'handling-boundary', label: 'confirm information handling and access boundaries', threshold: 0.48, explicitRequired: true, explicitPatterns: [/\b(?:handling|handled|access|acess|classification|permitted|allowed)\b/i], examples: ['Check whether sensitive lending information is permitted and who may access it.', 'Follow policy and access controls before moving any material.', 'Check what data and access rules apply before using it.', 'Confirm data classification, access control and handling requirements before transfer.', 'Confirm who can access the lending information and how it must be handled.', 'Check the information-handling and access rules before any upload.'] }
      ],
      ...
      contradictions: [{ id: 'proceed-while-unclear', patterns: ['upload the full customer file', 'upload the customer file', 'upload the file', 'use the public ai website', 'for a limited test', 'confirm approval afterwards', 'confirm approval afterward', 'confirm approval later', 'test it first', 'use it for a small test', 'run a small test', 'do a small test', 'try it anyway', 'proceed while', 'go ahead temporarily', 'go ahead while approval is pending', 'use the tool while approval is pending', 'treat silence as approval', 'treat that as permission', 'proceed before approval is confirmed', 'use it until told not to', 'remove the file name and proceed'] }],
      ...
      blockedFeedback: 'This would use customer information before the AI system is approved. Do not upload or test it. Confirm tool approval and the information-handling and access rules first.'
    }
  };
